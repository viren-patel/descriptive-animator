...
public class AnimationFileReader {
    ...
    while (sc.hasNext()) {
      ...
      switch (command) {
        ...
        case "scaleShape":
          ...
          break;
        case "rotate":
          RotateInfo roinfo = readRotateInfo(sc);
          builder.addRotate(
                  roinfo.getName(),
                  roinfo.getFrom(),
                  roinfo.getTo(),
  ...
  private RotateInfo readRotateInfo(Scanner sc) throws
    ...
    while (!info.isAllInitialized()) {
      ...
      switch (command) {
        case "rotateto":
          info.setFrom(sc.nextFloat());
          info.setTo(sc.nextFloat());
          break;
        case "name":
          info.setName(sc.next());
          break;
  ...
  class RotateInfo extends Inputable {
    ...
    RotateInfo() {
      ...
      valueFlags.put("name", false);
      valueFlags.put("from", false);
      valueFlags.put("to", false);
      valueFlags.put("start", false);
      valueFlags.put("end", false);
    ...
    void setFrom(float x) {
      this.from = x;
      valueFlags.replace("from", true);
    }

    void setTo(float x) {
      this.to = x;
      valueFlags.replace("to", true);
    }
